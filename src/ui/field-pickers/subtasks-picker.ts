...
interface SubtasksPickerOptions {
	value: string[];
	allTasks: IndexedTask[];
	excludedIds?: string[];
	closeOnSelect?: boolean;
	onChange: (operonIds: string[]) => void;
	onClose?: () => void;
}
...
export function showSubtasksPicker(anchor: HTMLElement | DOMRect, options: SubtasksPickerOptions): () => void {
	...
	input.placeholder = t('taskEditor', 'searchSubtasks');
	...
	const excludedIds = new Set((options.excludedIds ?? []).map(value => value.trim()).filter(Boolean));
	const initialSelectedIds = Array.from(new Set(options.value.map(value => value.trim()).filter(Boolean)));
	const taskById = new Map(options.allTasks.map(task => [task.operonId, task]));
	const allCandidates = buildCandidates(options.allTasks)
		.filter(candidate => {
			if (excludedIds.has(candidate.operonId)) return false;
			const task = taskById.get(candidate.operonId);
			const hasParent = !!(task?.fieldValues['parentTask'] ?? '').trim();
			return !hasParent || initialSelectedIds.includes(candidate.operonId);
		});
	const candidatesById = new Map(allCandidates.map(candidate => [candidate.operonId, candidate]));
	let selectedIds = initialSelectedIds.filter(operonId => candidatesById.has(operonId));
	...
	const getVisibleMatches = (): SubtaskCandidate[] => matches.slice(0, loadedCount);
...
function buildCandidates(tasks: IndexedTask[]): SubtaskCandidate[] {
...
function rankCandidates(candidates: SubtaskCandidate[], query: string): SubtaskCandidate[] {
	...
	return candidates
		.map(candidate => ({
			candidate,
			score: scoreSubtaskMatch(candidate, lowered),
		}))
...
function scoreSubtaskMatch(candidate: SubtaskCandidate, query: string): number {
